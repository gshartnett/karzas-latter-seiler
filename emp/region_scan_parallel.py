...
def contour_plot(x, y, z, Burst_Point, save_path=None, grid=False, show=True):
    ...
    for i in range(len(xi)):
        for j in range(len(yi)):
            ...
            Target_Point = geometry.Point(EARTH_RADIUS, phi, lambd, "lat/long geo", consistency_check=False)
            try:
...
def folium_plot(contourf, lat0, long0, levels, save_path, zoom_start=5, tiles:str="CartoDB positron"):
    """
    ...
    geomap = folium.Map(
        [lat0, long0], width=750, height=750, zoom_start=zoom_start, tiles=tiles,
    )
