...
from emp.constants import B0, EARTH_RADIUS
...
class DipoleMagneticField(MagneticField):
    ...
    def get_field_vector(
        ...
        if coordinate_system == CoordinateSystem.LATLONG_MAG:
            ...
            B_lambd = 0.0
            return np.array([B_r, B_phi, B_lambd])
            ...
            return np.asarray(B_r * r_hat + B_phi * phi_hat + B_lambd * lambd_hat)
            ...
            return np.array([B_r, B_theta, B_phi_az])
...
class MagneticFieldFactory:
    ...
    @classmethod
    def create(cls, model: Union[MagneticFieldModel, str], **kwargs) -> MagneticField:
